...
*   * Module method
*/
function nuxtPruneHtml(
    nuxt,
    moduleOptions
) {
    // When using defineNuxtModule, nuxt is always the Nuxt instance with options property
    if (!nuxt || !nuxt.options) {
        throw new Error('nuxt-prune-html: Invalid Nuxt instance. Expected nuxt.options to be available.');
    }
    ...
    */
    const options = {
        ...
        ... moduleOptions || {},
        ... nuxt.options[ moduleName ] || {},
        ... nuxt.options.pruneHtml || {},
    };
    ...
    *   * Triggers
    */
    options.hookRenderRoute && nuxt.hook(
        'render:route',
        async(
            _,
            result,
            ...
            */
            if( isTypeDefaultDetect && headers && headers[ options.headerNameForDefaultDetection ] ) {
                ...
                const MobileDetect = require( 'mobile-detect' )
                    ...
                    , isAuditExtra = (
                        options.isAuditExtra
                        && ( options.auditUserAgentExtra && options.auditUserAgentExtra.length )
                        && stringToArray( options.auditUserAgentExtra ).some( v => new MobileDetect(
                            headers[ options.headerNameForDetection ],
                        ).match( v ) )
                    )
                    , isUserAgentMatched = (
    ...
    options.hookGeneratePage && nuxt.hook(
        'generate:page',
        async result => {
...
// Export as Nuxt 3 module
export default defineNuxtModule({
    meta,
    setup(moduleOptions, nuxt) {
        // defineNuxtModule setup receives: (moduleOptions, nuxt)
        // nuxtPruneHtml expects: (nuxt, moduleOptions)
        // Correctly map: nuxt (setup's 2nd param) -> nuxtPruneHtml's 1st arg
        //                moduleOptions (setup's 1st param) -> nuxtPruneHtml's 2nd arg
        return nuxtPruneHtml(nuxt, moduleOptions);
    }
});
